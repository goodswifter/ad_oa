...
class ContractInvoiceHistory extends StatefulWidget {
  ContractInvoiceHistory({Key? key}) : super(key: key);

  @override
  _ContractInvoiceHistoryState createState() => _ContractInvoiceHistoryState();
}

class _ContractInvoiceHistoryState extends State<ContractInvoiceHistory> {
  List<InvoiceHistoryItems> data = [];
  ...
  void loadMore() {
    WorkSpaceService().getInvoiceGetPagedListForContract(
  ...
  Widget listItem({InvoiceHistoryItems? model}) {
    return Card(
      color: Colors.white,
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 6),
            Text("开票时间：${model.invoiceDate ?? "暂无"}"),
            SizedBox(height: 6),
            Text("发票类别：${model.invoiceTypeName ?? "暂无"}"),
            SizedBox(height: 6),
